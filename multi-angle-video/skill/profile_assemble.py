...
def _face_xy(mp4):
    """nose-x / eye-y fraction in the HOME shot → keep eyes at the same screen height through the
    zoom (no eyeline jump). Fallback (0.5, 0.42) = centered, eyes on the upper third."""
    try:
        import cv2
...
ENC = ["-c:v","libx264","-preset","medium","-crf","20","-pix_fmt","yuv420p","-r","30",
       "-vf","scale=720:1280:force_original_aspect_ratio=increase,crop=720:1280","-an"]
